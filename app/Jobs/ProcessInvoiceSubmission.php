...
class ProcessInvoiceSubmission implements ShouldQueue
{
     ...
     */
    public function __construct($invoiceId, $token, $merchantId)
    {
        $this->invoiceId = $invoiceId;
        $this->token = $token;
        $this->merchantId = $merchantId;
        $this->queue = 'submit-invoice';
    ...
    public function handle(): void
    {
        ...
        $invoice = Invoice::where('invoice_no', $this->invoiceId)
                ...
                ->first();
        $merchantDetail = Merchant::find($this->merchantId);
        $now = Carbon::now();
        $payout = PayoutConfig::where('merchant_id', $this->merchantId)->first();
        $msic = MSICcode::find($merchantDetail->msic_id);
        $state = State::where('State', $merchantDetail->state_code)->first();
        $eCode = md5($invoice->invoice_no . $this->merchantId . $payout->secret_key);
